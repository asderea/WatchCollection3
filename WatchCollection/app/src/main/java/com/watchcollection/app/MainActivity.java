...
public class MainActivity extends Activity {
    ...
    private View buildWatchSummarySpecs(Watch w) {
        ...
        if(!w.diameterMm.isEmpty()) extra.add("Ø "+w.diameterMm+" mm");
        if(!w.lugToLugMm.isEmpty()) extra.add("L2L "+w.lugToLugMm+" mm");
        if(!w.waterResistance.isEmpty()) {
            String wr=w.waterResistance.trim();
            extra.add(wr.toUpperCase(Locale.ROOT).startsWith("WR")?wr:"WR "+wr);
        }
        if(!extra.isEmpty()) {
            TextView tv=UiKit.muted(this,String.join("  |  ",extra),11);
            tv.setPadding(0,UiKit.dp(this,1),0,0);
            box.addView(tv);
            shown++;
        }
    ...
    private void showWatchAccuracyMeterResultDialog(long defaultWatchId) {
        ...
        for(int i=0;i<positionItems.length;i++) {
            ...
            rb.setText(positionIconText(key)+"  "+positionItems[i][1]);
            rb.setTag(key);
    ...
    private void refreshPositionSummary(LinearLayout container,long watchId) {
        ...
        for(String p:positions) {
            ...
            TextView name=UiKit.text(this,positionIconText(p)+"  "+positionLabel(p),12,true);
    ...
    private void showAllWatchAccuracyPopup() {
        ...
        for(Watch w:watches) {
            ...
            for(String pos:order) {
                ...
                LinearLayout line=UiKit.row(this);
                line.addView(UiKit.text(this,positionIconText(pos)+"  "+positionLabel(pos),11,true),new LinearLayout.LayoutParams(0,ViewGroup.LayoutParams.WRAP_CONTENT,1));
    ...
    private void showWatchAccuracyPopup(Watch w) {
        ...
        for(String p:positions) {
            ...
            for(DatabaseHelper.PositionAverage r:rows) if(p.equals(r.position)){found=r;break;}
            LinearLayout line=UiKit.row(this);
            line.addView(UiKit.text(this,positionIconText(p)+"  "+positionLabel(p),12,true),new LinearLayout.LayoutParams(0,ViewGroup.LayoutParams.WRAP_CONTENT,1));
            String value="—";
            if(found!=null) {
    ...
    private View buildCollectionPhotosSection() {
        ...
        LinearLayout top=UiKit.row(this);
        top.addView(UiKit.text(this,(collectionPhotoIndex+1)+" / "+photos.size(),13,true),new LinearLayout.LayoutParams(0,ViewGroup.LayoutParams.WRAP_CONTENT,1));
        if(photos.size()>1) {
            Button prev=UiKit.secondaryButton(this,"‹");
            Button next=UiKit.secondaryButton(this,"›");
        ...
        card.addView(image,new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,UiKit.dp(this,210)));
    ...
    private String positionIconText(String position) {
        if("Dial up".equals(position)) return "◎";
        if("Dial down".equals(position)) return "◉";
        if("12 Up".equals(position)) return "12↑";
        if("9 Up".equals(position)) return "9↑";
        if("6 Up".equals(position)) return "6↑";
        if("3 Up".equals(position)) return "3↑";
        return "•";
    }
