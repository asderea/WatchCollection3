...
public class MainActivity extends Activity {
    ...
    private int strapWidthFilter = 0; // 0 All, 18~22 mm, -1 기타
    ...
    private View watchCard(Watch w, boolean focused) {
        ...
        Button accuracy=UiKit.secondaryButton(this,"오차");
        ...
        accuracy.setOnClickListener(v->showWatchAccuracyPopup(w));
        ...
        actions.addView(accuracy,new LinearLayout.LayoutParams(0,ViewGroup.LayoutParams.WRAP_CONTENT,1));
    ...
    private String joinSpecs(Watch w) {
        ...
        String power=normalizePower(w.movement);
        if(!power.isEmpty()) s.add("Power "+power);
        if(!w.caliber.isEmpty()) s.add("Movement "+w.caliber);
        ...
        if(!w.waterResistance.isEmpty()) {
            String wr=w.waterResistance.trim();
            s.add(wr.toUpperCase(Locale.ROOT).startsWith("WR")?wr:"WR "+wr);
        }
    ...
    private void showWatchDialog(Watch existing) {
        ...
        form.addView(UiKit.label(this,"Power"));
        Spinner power=powerSpinner(base.movement);
        form.addView(power);
        EditText caliber=addField(form,"Movement","예: H-10 / 6R54 / E168",base.caliber);
        ...
        EditText water=addField(form,"WR","예: 100 m",base.waterResistance);
        ...
        form.addView(UiKit.muted(this,"보증 만료일을 누르면 착용 캘린더와 별개의 날짜 선택 팝업이 열립니다.",10));
        ...
        dlg.setOnShowListener(x->dlg.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener(v->{
            ...
            base.caseMaterial=val(material); String warrantyValue=val(warranty); base.warrantyEndDate=warrantyValue.startsWith("~")?warrantyValue.substring(1):warrantyValue; base.sourceUrls=val(sources); base.notes=val(notes);
    ...
    private void showAccuracy() {
        ...
        overview.addView(UiKit.muted(this,"모든 시계의 전체 Rate 평균과 6개 자세별 Rate 평균을 사진과 함께 한 번에 확인합니다.",11));
        overview.addView(UiKit.spacer(this,8));
        Button allAccuracy=UiKit.primaryButton(this,"전체 시계 Rate 평균 보기");
        allAccuracy.setOnClickListener(v->showAllWatchAccuracyPopup());
        overview.addView(allAccuracy);
        p.addView(overview);

        LinearLayout wamCard=UiKit.card(this);
        wamCard.addView(UiKit.eyebrow(this,"WATCH ACCURACY METER"));
        wamCard.addView(UiKit.text(this,"외부 측정 연동",18,true));
    ...
    private void refreshPositionSummary(LinearLayout container,long watchId) {
        ...
        for(String p:positions) {
            ...
            if(found==null) {
                ...
                value=String.format(Locale.KOREA,"%+.1f s/d · %.2f ms%s",
                        found.avgRateSecDay,found.avgBeatErrorMs,amp);
            }
    ...
    private void showAllWatchAccuracyPopup() {
        ...
        for(Watch w:watches) {
            ...
            for(String pos:order) {
                ...
                LinearLayout line=UiKit.row(this);
                line.addView(UiKit.text(this,positionLabel(pos),11,true),new LinearLayout.LayoutParams(0,ViewGroup.LayoutParams.WRAP_CONTENT,1));
    ...
    private View rateOverviewGraph(List<Watch> watches) {
        LinearLayout card=UiKit.card(this);
        card.addView(UiKit.eyebrow(this,"RATE COMPARISON"));
        card.addView(UiKit.text(this,"Rate 평균 그래프",17,true));
        card.addView(UiKit.muted(this,"가운데 기준선이 0 s/day입니다. 막대 길이는 전체 시계 중 가장 큰 |Rate|를 기준으로 상대 표시합니다.",10));
        card.addView(UiKit.spacer(this,8));
        ...
        final int plotWidth=UiKit.dp(this,118);
        final int half=plotWidth/2;
        for(Watch w:watches) {
            double r=db.averageTimegrapherRateForWatch(w.id);
            LinearLayout row=UiKit.row(this);
            row.setGravity(Gravity.CENTER_VERTICAL);
            ImageView image=watchThumb(w.representativeImageUrl,34);
            row.addView(image,new LinearLayout.LayoutParams(UiKit.dp(this,34),UiKit.dp(this,34)));

            TextView name=UiKit.text(this,w.modelName,10,true);
            name.setMaxLines(2);
            LinearLayout.LayoutParams nameLp=new LinearLayout.LayoutParams(UiKit.dp(this,86),ViewGroup.LayoutParams.WRAP_CONTENT);
            nameLp.setMargins(UiKit.dp(this,6),0,UiKit.dp(this,5),0);
            row.addView(name,nameLp);

            FrameLayout plot=new FrameLayout(this);
            View zero=new View(this);
            zero.setBackgroundColor(UiKit.LINE);
            FrameLayout.LayoutParams zlp=new FrameLayout.LayoutParams(UiKit.dp(this,1),UiKit.dp(this,24));
            zlp.leftMargin=half;
            zlp.gravity=Gravity.CENTER_VERTICAL;
            plot.addView(zero,zlp);
            if(!Double.isNaN(r)) {
                int barWidth=Math.max(UiKit.dp(this,2),(int)Math.round((Math.abs(r)/maxAbs)*(half-UiKit.dp(this,3))));
                View bar=new View(this);
                bar.setBackground(UiKit.rounded(r>=0?UiKit.GOLD:UiKit.NAVY_2,Color.TRANSPARENT,5,this));
                FrameLayout.LayoutParams blp=new FrameLayout.LayoutParams(barWidth,UiKit.dp(this,8));
                blp.leftMargin=r>=0?half:half-barWidth;
                blp.gravity=Gravity.CENTER_VERTICAL;
                plot.addView(bar,blp);
            }
            row.addView(plot,new LinearLayout.LayoutParams(plotWidth,UiKit.dp(this,30)));

            TextView rate=UiKit.text(this,Double.isNaN(r)?"—":String.format(Locale.KOREA,"%+.1f",r),10,true);
            rate.setGravity(Gravity.END|Gravity.CENTER_VERTICAL);
            rate.setTextColor(Double.isNaN(r)?UiKit.MUTED:UiKit.NAVY);
            row.addView(rate,new LinearLayout.LayoutParams(0,ViewGroup.LayoutParams.WRAP_CONTENT,1));
            row.setPadding(0,UiKit.dp(this,3),0,UiKit.dp(this,3));
            row.setOnClickListener(v->showWatchAccuracyPopup(w));
            card.addView(row);
        }
        return card;
    }
    ...
    private void showWatchAccuracyPopup(Watch w) {
        ...
        for(String p:positions) {
            ...
            for(DatabaseHelper.PositionAverage r:rows) if(p.equals(r.position)){found=r;break;}
            LinearLayout line=UiKit.row(this);
            line.addView(UiKit.text(this,positionLabel(p),12,true),new LinearLayout.LayoutParams(0,ViewGroup.LayoutParams.WRAP_CONTENT,1));
            String value="—";
            if(found!=null) {
    ...
    private void showStraps(int filterWidth) {
        ...
        String[] widthItems={"All","18 mm","19 mm","20 mm","21 mm","22 mm","기타"};
        ...
        String[] materialItems={"All","Leather","Bracelet","Rubber","Nato","Mesh","기타"};
    ...
    private void showStrapDialog(Strap existing) {
        ...
        String[] materialItems={"Leather","Bracelet","Rubber","Nato","Mesh","기타"};
