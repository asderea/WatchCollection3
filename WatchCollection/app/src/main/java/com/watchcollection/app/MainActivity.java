...
public class MainActivity extends Activity {
    ...
    private int monthlyWearPage = 0; // 3개씩 표시: 0=1~3위, 1=4~6위, 2=7~9위, 3=10위
    ...
    private void showCollection() {
        ...
        if(!allWatches.isEmpty()) {
            ...
            p.addView(UiKit.label(this,"Power"));
            LinearLayout filterRow=UiKit.row(this);
            String[] labels={"All","Automatic","Mechanical","Quartz"};
    ...
    private View monthlyWearSummary() {
        ...
        int capped=Math.min(10,entries.size());
        int totalPages=Math.max(1,(capped+2)/3);
        ...
        int to=Math.min(Math.min(from+3,10),entries.size());
    ...
    private View brandFilterView(List<Watch> watches) {
        ...
        box.addView(UiKit.label(this,"Brand"));
    ...
    private void showAccuracy() {
        ...
        p.addView(UiKit.muted(this,"Watch Accuracy Meter에서 측정한 값을 자세별로 저장하고, 시계별 평균 Rate와 자세별 평균을 한눈에 관리합니다.",13));
        p.addView(UiKit.spacer(this,12));
        ...
        wamCard.addView(UiKit.muted(this,"Watch Accuracy Meter에서 측정한 뒤 Watch Collection으로 돌아오면 결과 입력 팝업이 자동으로 열립니다. 공개 결과 API가 없어 값은 화면을 보고 한 번 입력해야 합니다.",11));
    ...
    private void refreshPositionSummary(LinearLayout container,long watchId) {
        ...
        for(String p:positions) {
            ...
            if(found==null) {
                ...
                value=String.format(Locale.KOREA,"%+.1f s/d · %.2f ms%s · n=%d",
    ...
    private void showAllWatchAccuracyPopup() {
        ...
        body.addView(UiKit.muted(this,"전체 Rate 평균은 저장된 모든 측정값 기준이며, 자세 평균은 기록이 있는 자세들의 평균 Rate를 동일 가중치로 계산합니다.",11));
        body.addView(UiKit.spacer(this,8));

        for(Watch w:watches) {
            LinearLayout card=UiKit.card(this);
            LinearLayout head=UiKit.row(this);
            ImageView image=watchThumb(w.representativeImageUrl,68);
            head.addView(image,new LinearLayout.LayoutParams(UiKit.dp(this,68),UiKit.dp(this,68)));
            LinearLayout info=UiKit.column(this);
            ...
            for(String pos:order) {
                ...
                String value=found==null?"—":String.format(Locale.KOREA,"%+.2f s/d · n=%d",found.avgRateSecDay,found.count);
