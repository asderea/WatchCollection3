...
public class MainActivity extends Activity {
    ...
    private void showAccuracy() {
        ...
        p.addView(UiKit.muted(this,"Watch Accuracy Meter를 기본 측정 도구로 연결하고, 측정 결과를 자세별 기록으로 저장할 수 있습니다. 자체 타임그래퍼는 실험적 보조 기능으로 유지합니다.",13));
        p.addView(UiKit.spacer(this,12));
        ...
        Button quickAccuracy=UiKit.secondaryButton(this,"선택 시계 오차 한눈에 보기");
        quickAccuracy.setOnClickListener(v->{
            int idx=timegrapherWatchSpinner==null?-1:timegrapherWatchSpinner.getSelectedItemPosition();
            if(idx>=0&&idx<watches.size()) showWatchAccuracyPopup(watches.get(idx));
        });
        setup.addView(UiKit.spacer(this,8));
        setup.addView(quickAccuracy);
        p.addView(setup);
        ...
        meterCard.addView(UiKit.eyebrow(this,"EXPERIMENTAL"));
        meterCard.addView(UiKit.text(this,"자체 타임그래퍼",16,true));
        meterCard.addView(UiKit.muted(this,"휴대폰 마이크 기반 자체 측정입니다. Watch Accuracy Meter보다 기기별 편차가 클 수 있습니다.",11));
        meterCard.addView(UiKit.spacer(this,8));
        ...
        meterCard.addView(UiKit.label(this,"Lift angle (°) · amplitude 계산에 사용"));
        meterCard.addView(timegrapherLiftAngle);
        TextView auto=UiKit.muted(this,"BPH AUTO 후보 · "+TimegrapherEngine.commonBphText()+" bph",11);
        auto.setPadding(0,UiKit.dp(this,4),0,UiKit.dp(this,4));
        meterCard.addView(auto);
        meterCard.addView(UiKit.muted(this,"약 2~4초부터 BPH/Rate 잠정값이 표시되고, BPH Lock 후 약한 tick도 예상 시점에서 추적합니다. 8초 이후 안정적인 값은 저장할 수 있습니다. amplitude는 신호가 불충분하면 '-'로 남습니다.",11));
        meterCard.addView(UiKit.spacer(this,8));
    ...
    private void updateTimegrapherUi(TimegrapherEngine.Snapshot snapshot) {
        ...
        if(timegrapherStatus!=null) {
            ...
            if(snapshot.bph>0 && snapshot.bphLocked) bph += " · LOCK";
    ...
    private void showWatchAccuracyPopup(Watch w) {
        ...
        List<DatabaseHelper.PositionAverage> rows=db.getPositionAveragesForWatch(w.id);
        String[] positions={"Dial up","Dial down","12 Up","9 Up","6 Up","3 Up"};
        for(String p:positions) {
            DatabaseHelper.PositionAverage found=null;
            for(DatabaseHelper.PositionAverage r:rows) if(p.equals(r.position)){found=r;break;}
        ...
        if(recent.isEmpty()) {
            latest.addView(UiKit.muted(this,"아직 저장된 타임그래퍼 측정값이 없습니다.",12));
        } else {
            for(DatabaseHelper.TimegrapherRow r:recent) {
                LinearLayout line=UiKit.row(this);
                String srcLabel="watch_accuracy_meter".equals(r.source)?"WAM":"자체";
    ...
    private View timegrapherRecordCard(DatabaseHelper.TimegrapherRow r) {
        ...
        names.addView(UiKit.muted(this,r.date+" · "+positionLabel(r.position)+" · "+(fromWam?"Watch Accuracy Meter":"자체 측정"),11));
