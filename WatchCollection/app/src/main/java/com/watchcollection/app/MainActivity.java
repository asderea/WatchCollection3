...
public class MainActivity extends Activity {
    ...
    private View buildWatchSummarySpecs(Watch w) {
        ...
        if(!w.diameterMm.isEmpty()) { box.addView(boldValueLine("Φ", w.diameterMm+" mm", 12)); shown++; }
        if(!w.lugToLugMm.isEmpty()) { box.addView(boldValueLine("L2L", w.lugToLugMm+" mm", 12)); shown++; }
        if(!w.lugWidthMm.isEmpty()) { box.addView(boldValueLine("Lug", w.lugWidthMm+" mm", 12)); shown++; }
        if(!w.waterResistance.isEmpty()) {
            String wr=w.waterResistance.trim();
            box.addView(boldValueLine("WR", wr.toUpperCase(Locale.ROOT).startsWith("WR")?wr.substring(Math.min(2,wr.length())).trim():wr, 12));
            shown++;
        }
    ...
    private View buildCollectionPhotosSection() {
        ...
        LinearLayout.LayoutParams frameLp=new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,UiKit.dp(this,226));
        card.addView(frame,frameLp);

        ImageView image=new ImageView(this);
        image.setScaleType(ImageView.ScaleType.CENTER_CROP);
        image.setAdjustViewBounds(false);
        ImageLoader.load(this,image,current.displayPath);
        ...
        count.setBackground(UiKit.rounded(Color.argb(185,22,32,51),Color.TRANSPARENT,12,this));
        count.setPadding(UiKit.dp(this,10),UiKit.dp(this,5),UiKit.dp(this,10),UiKit.dp(this,5));
        FrameLayout.LayoutParams countLp=new FrameLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT,ViewGroup.LayoutParams.WRAP_CONTENT,Gravity.TOP|Gravity.CENTER_HORIZONTAL);
        countLp.topMargin=UiKit.dp(this,9);
        frame.addView(count,countLp);

        if(photos.size()>1) {
            Button prev=overlayArrowButton("‹");
            Button next=overlayArrowButton("›");
            ...
            FrameLayout.LayoutParams plp=new FrameLayout.LayoutParams(UiKit.dp(this,46),UiKit.dp(this,42),Gravity.TOP|Gravity.START);
            plp.setMargins(UiKit.dp(this,8),UiKit.dp(this,7),0,0);
            frame.addView(prev,plp);
            FrameLayout.LayoutParams nlp=new FrameLayout.LayoutParams(UiKit.dp(this,46),UiKit.dp(this,42),Gravity.TOP|Gravity.END);
            nlp.setMargins(0,UiKit.dp(this,7),UiKit.dp(this,8),0);
            frame.addView(next,nlp);
        }
        return card;
    }
    ...
    private void showWatchDetailPopup(Watch w) {
        ...
        AlertDialog.Builder builder=new AlertDialog.Builder(this)
                ...
                .setView(scroll)
                .setPositiveButton("닫기",null);
        if(!w.sourceUrls.isEmpty()) builder.setNeutralButton("출처",(d,x)->openFirstSource(w.sourceUrls));
        builder.show();
    }
