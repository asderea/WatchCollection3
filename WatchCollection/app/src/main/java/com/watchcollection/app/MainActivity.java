...
public class MainActivity extends Activity {
    ...
    private void showAccuracy() {
        ...
        p.addView(UiKit.muted(this,"휴대폰 마이크로 기계식 시계의 틱/톡을 분석합니다. 멀티밴드 + BPH Lock으로 약한 신호도 빠르게 추적합니다.",13));
        p.addView(UiKit.spacer(this,12));
        ...
        setup.addView(auto);
        setup.addView(UiKit.muted(this,"약 2~4초부터 BPH/Rate 잠정값이 표시되고, BPH Lock 후 약한 tick도 예상 시점에서 추적합니다. 8초 이후 안정적인 값은 저장할 수 있습니다. amplitude는 신호가 불충분하면 '-'로 남습니다.",11));
    ...
    private void showWatchAccuracyPopup(Watch w) {
        ...
        if(recent.isEmpty()) {
            ...
            for(DatabaseHelper.TimegrapherRow r:recent) {
                LinearLayout line=UiKit.row(this);
                TextView left=UiKit.text(this,r.date+" · "+positionLabel(r.position),11,false);
