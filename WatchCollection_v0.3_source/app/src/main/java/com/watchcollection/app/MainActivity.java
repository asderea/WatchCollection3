...
public class MainActivity extends Activity {
    ...
    private void showCollection() {
        ...
        if(allWatches.isEmpty()) {
            p.addView(emptyCard("첫 시계를 등록해보세요","모델명과 레퍼런스를 입력하면 관련 없는 결과를 제거한 뒤 공식 페이지를 우선해 스펙과 대표사진을 찾습니다."));
        } else {
    ...
    private void showWatchDialog(Watch existing) {
        ...
        Button lookup=UiKit.primaryButton(this,"정밀 검색 · 스펙 + 공식 대표사진");
        form.addView(lookup);
        TextView status=UiKit.muted(this,"브랜드/레퍼런스를 함께 입력하면 해당 제조사 공식 페이지와 정확한 모델을 더 강하게 우선합니다. 관련 없는 결과는 자동 폐기합니다.",12);
        ...
        final String[] imageUrl={base.representativeImageUrl};
        final String[] imageSource={base.imageSourceUrl};
        lookup.setOnClickListener(v->{
            String q=val(model);
            if(q.isEmpty()){model.setError("모델명을 입력하세요.");return;}
            lookup.setEnabled(false);
            status.setText("공식 도메인 우선 검색 → 관련성 점수화 → 페이지 본문 모델 검증 → 스펙 추출 중…");
            executor.execute(()->{
                try {
                    ...
                    runOnUiThread(()->{
                        ...
                        imageUrl[0]=r.representativeImageUrl;
                        imageSource[0]=r.imageSourceUrl;
                        ImageLoader.load(this,preview,imageUrl[0]);
        ...
        dlg.setOnShowListener(x->dlg.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener(v->{
            ...
            base.caseMaterial=val(material); base.sourceUrls=val(sources); base.representativeImageUrl=imageUrl[0];
            base.imageSourceUrl=imageSource[0]; base.notes=val(notes);
            long savedId=db.saveWatch(base);
            dlg.dismiss();
            focusedWatchId=savedId;
            showCollection();

            if(base.representativeImageUrl==null || base.representativeImageUrl.trim().isEmpty()) {
                Toast.makeText(this,"공식 대표사진을 정밀 검색 중입니다.",Toast.LENGTH_SHORT).show();
                executor.execute(()->{
                    try {
                        WebSpecLookup.Result auto=WebSpecLookup.lookup(base.modelName,base.brand,base.referenceNo);
                        if(auto.representativeImageUrl!=null&&!auto.representativeImageUrl.isEmpty()) {
                            Watch saved=db.getWatch(savedId);
                            if(saved!=null) {
                                saved.representativeImageUrl=auto.representativeImageUrl;
                                saved.imageSourceUrl=auto.imageSourceUrl;
                                if(saved.sourceUrls.isEmpty()) saved.sourceUrls=auto.sourceUrls;
                                db.saveWatch(saved);
                                runOnUiThread(this::showCollection);
                            }
                        }
                    } catch(Exception ignored) {}
                });
